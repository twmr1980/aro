...
if(is_writeable("invupd.log")){ 
	...
	//$resultItemName = $link->query($sqlItemName);
	$res2 = mysqli_fetch_row($link->query($sqlItemName));
	$displayName = $res2[0];
	...
	if ($link->query($updLevel) === TRUE) {
		if($countRes == 1) {
			...
			$logTxt = ($countInvLog+1).". ". $displayName .", ".$itemId.", ". $amountAdj.", ". $tStamp. ",  ".$countRes. $res[0]."\n";
			fwrite($fh, $logTxt);
		} elseif ($countRes == 0){
			$tStamp = date('Y-m-d h:i:sa');
